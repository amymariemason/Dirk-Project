...
for (i in 1:length(TL)){
  ...
  output<-read.table(paste("./Inputs/GWAS_HESoutcomes_", outcomespec, "-output.out", sep = ""), header=T)
  ...
  beta.ratio<-by/bx
  se.ratio<-byse/sqrt(bx^2)
  
  cat("MR analysis results \n")
  print(beta.ratio)
  print
  summary_temp<-list(outcomespec, outcomename,beta.ratio,se.ratio)
...
input <- data.frame(
  ...
  beta = sapply(list_save, "[[", 3),
  se = sapply(list_save,"[[",4)   
)
  
res <- rma(yi=input$beta, sei=input$se, method="FE")
